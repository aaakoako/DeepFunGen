...
function getMessages() {
    const locale = State.get('language') || 'en';
    ...
    return {
        ...
        update: isKo
            ...
            ? {
                checking: '正在检查更新...',
                upToDate: (latest) => (latest ? `已是最新版本 (最新 ${latest})` : '已是最新版本'),
                available: (latest) => (latest ? `有新更新: ${latest}` : '有新更新可用'),
                error: (reason) => (reason ? `检查失败: ${reason}` : '无法检查更新。'),
                link: '查看发布',
                toastAvailable: (latest) => (latest ? `有新更新: ${latest}` : '有新更新可用。'),
                toastUpToDate: '您已是最新版本。',
            }
            : {
