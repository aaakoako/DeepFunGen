<<<<<<< HEAD:DeepFunGen.py/backend/routes.py
...
from .video_pipeline import resolve_script_path, resolve_prediction_path
...
async def recommend_parameters_endpoint(
    ...
    # If no CSV exists, we need to perform inference
    # For now, we'll require the CSV to exist for parameter recommendation
    if df is None or len(df) == 0:
        raise HTTPException(
            status_code=400,
            detail="Prediction file not found. Please process the video first to generate predictions."
        )
